...
@Service
public class EighthExcelFileServiceImpl implements EighthExcelFileService {

    private final EighthSemesterInterface eighthRepository;

    @Autowired
    public EighthExcelFileServiceImpl(EighthSemesterInterface eighthRepository) {
        this.eighthRepository = eighthRepository;
    }

    @Override
    public List<eighth> processExcelFile(MultipartFile file) throws IOException {
        if (file.isEmpty()) {
            throw new IOException("The supplied file is empty.");
        }

        List<eighth> entities = new ArrayList<>();
        ...
        try (Workbook workbook = WorkbookFactory.create(file.getInputStream())) {
            ...
            eighthRepository.saveAll(entities);
    ...
    private void populateEntityFromRow(eighth entity, Row row) {
