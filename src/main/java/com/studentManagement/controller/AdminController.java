...
//				return createStudentForm(userId, m, session, redirectAttributes);
	...
	public String createTeacherForm(@RequestParam String userId, Model model, HttpSession s, RedirectAttributes r)
	{
		return "create_teacher1";
	}
	@PostMapping("/students/new")
	public String createStudentForm(@RequestParam String userId, Model model, HttpSession s, RedirectAttributes r)
	{
		...
		s.setAttribute("msg", "student created successfully");
		r.addAttribute("userId", userId);
		r.addFlashAttribute("message", "Student Register successfully");
		return "create_student";
	}

	//	save teacher
	@PostMapping("/saveTeacher")
	public String saveTeacher(@ModelAttribute("teacher") Teacher teacher, Model m, Principal principal) {
		User use1 = new User();
		use1.setUserId(teacher.getTeacherId());
		use1.setName(teacher.getName());
		use1.setRole("ROLE_ADMIN");
		use1.setPassword(teacher.getPassword());
		userRepo.save(use1);
	...
	public String saveStudent(@ModelAttribute("student") Student  student, Model m, Principal principal)
	{
		User use1 = new User();
		use1.setUserId(student.getId());
		String str1=student.getFirstName();
		String str2=student.getLastName();
		use1.setName(str1+" "+str2);
		use1.setRole("ROLE_USER");
		use1.setPassword(student.getPassword());
		userRepo.save(use1);
